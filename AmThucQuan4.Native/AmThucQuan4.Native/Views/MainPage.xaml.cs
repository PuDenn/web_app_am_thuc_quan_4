...
public partial class MainPage : ContentPage
{
    ...
    public MainPage(MainViewModel vm, IApiService api,
                    ILanguageService lang, IQRService qr)
    {
        InitializeComponent();
    ...
    private async void OnShowQrClicked(object? sender, EventArgs e)
    {
        ...
        // Guest mode: audio cần quét QR → hướng dẫn rõ hơn
        var isGuest = _api.IsGuest;
        var title   = isGuest
            ? $"📱 Quét QR để nghe thuyết minh"
            : $"📱 Xem {poi.Name} trên điện thoại";

        var qrSource = _qr.GenerateQr(poi.Id);
        var webUrl   = _qr.GetPoiWebUrl(poi.Id);
        ...
        var popup = new ContentPage
        {
            ...
            Content = new Frame
            {
                BackgroundColor  = Microsoft.Maui.Graphics.Color.FromArgb("#1e293b"),
                CornerRadius     = 20,
                ...
                Content = new VerticalStackLayout
                {
                    ...
                    Children =
                    {
                        new Label
                        {
                            ...
                            TextColor         = Microsoft.Maui.Graphics.Color.FromArgb("#f97316"),
                            HorizontalOptions = LayoutOptions.Center,
                        },
                        new Image
                        {
                        ...
                        {
                            Text = isGuest
                                ? "Dùng điện thoại quét mã QR\nđể nghe thuyết minh tại quán này"
                                : "Dùng điện thoại quét mã QR\nđể xem thông tin + audio + bản đồ",
                            FontSize          = 13,
                        ...
                        new Label
                        {
                            Text              = webUrl,
                            FontSize          = 10,
                            TextColor         = Microsoft.Maui.Graphics.Color.FromArgb("#475569"),
                            HorizontalOptions = LayoutOptions.Center,
                        ...
                        {
                            Text            = "✕ Đóng",
                            BackgroundColor = Microsoft.Maui.Graphics.Color.FromArgb("#334155"),
        ...
        // Gắn sự kiện đóng cho nút cuối
        var stack  = (VerticalStackLayout)((Frame)popup.Content).Content;
        var btnClose = (Button)stack.Children[^1];
        btnClose.Clicked += async (_, _) => await Navigation.PopModalAsync();
